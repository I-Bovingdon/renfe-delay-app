...
TRES COSAS QUE EL FEED NO DA Y HAY QUE DERIVAR
  ...
  1. `bearing`, `speed` y `currentStopSequence` vienen 100% nulos, verificado sobre los
     ...
     coordenadas están en el catálogo. Con el tren parado en andén, la próxima parada es
     la SIGUIENTE del recorrido, o el rumbo sería cero.
...
def rumbo_grados(lat1: float, lon1: float, lat2: float, lon2: float) -> float | None:
    ...
    Devuelve None si los dos puntos son prácticamente el mismo: un rumbo calculado
    sobre una distancia de metros es ruido, y una flecha girada al azar es peor que
    ninguna flecha.
    """
...
class FuentePosiciones:
    ...
    def _parsear(self, ruta: Path) -> list[dict[str, Any]]:
        ...
        for entidad in entidades:
            ...
            # --- Rumbo hacia la próxima parada ---
            objetivo = stop_id
            if paradas and stop_id in paradas:
                i = paradas.index(stop_id)
                # Parado en andén: el stop_id es la parada ACTUAL. Apuntar a ella
                # daría rumbo nulo, así que se apunta a la siguiente del recorrido.
                if estado == ESTADO_PARADO and i + 1 < len(paradas):
                    objetivo = paradas[i + 1]
            est_objetivo = self.cat.estacion(objetivo)
            rumbo = (
